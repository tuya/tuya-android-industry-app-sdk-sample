package com.tuya.dev.common.kv;
...
public class KvGlobalManager {
    ...
    public static MMKV getMMKVManager() {
        if (sMMKVManager == null) {
            synchronized (KvGlobalManager.class) {
                if (sMMKVManager == null) {
                    Context appContext = getSystemApp();
                    MMKV.initialize(appContext);

                    sMMKVManager = MMKV.mmkvWithID("KvGlobalId", MMKV.SINGLE_PROCESS_MODE, GLOBAL_KEY);
                }
            }
        }
        return sMMKVManager;
